...
#' @param method The confounder correction method to use. Both "ols" and "theory" use the bias-corrected method described in \url{https://arxiv.org/abs/1801.00865v2}, where "ols" weights units by their inverse variance. If neither of these methods is used, we recommend using "huber".
...
#' @export
Correction <- function(Y, X, ind.cov, r.confound, max.iter.svd=1, method=c("ols", "theory", "huber", "naive", "none"), shrink.Sigma=F, min.true.eig=0.1, svd.method = "slow") {
	p <- nrow(Y)
	n <- ncol(Y)
	d <- ncol(X)
  
  method <- match.arg(method, c("ols", "theory", "huber", "naive", "none"))  #Default is ols
  ...
  if (method != "none" && r.confound > 0) {
	...
	return(list(B=B, Sigma=Sigma.shrink, p.values=p.values, p.values.ols=p.values.ols, p.values.f=pvalues.f, z.scores=qnorm( pt(t,df=n-d-r.confound) ), z.scores.ols=qnorm( pt(t.ols,df=n-d) ), C=C, L=L.svd, Omega=Omega.ols, all.Omegas=Omegas, Cov.total=cbind(X,C), method=method, X=X, ind.cov=ind.cov))
}
...
#' @export
ChangeMethod <- function(Corr.Object, new.method=c("ols", "theory", "huber", "naive", "none")) {
  X <- Corr.Object$X
  ...
  new.method <- match.arg(new.method, c("ols", "theory", "huber", "naive", "none"))  #Default is ols
